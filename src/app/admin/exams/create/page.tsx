...
interface SectionConfig {
  name: string;
  subject: string; // links section → question subject for filtering
  duration: number;
...
export default function CreateExamWizard() {
  ...
  const [instructions, setInstructions] = useState(
    "1. This exam features multiple-choice questions (MCQs).\n2. Each section has a specific time limit. You cannot return to a section once the time limit expires.\n3. Make sure to review the marking schemes for negative marking details."
  );

  // Dynamic categories + subjects loaded from DB
  const [dbCategories, setDbCategories] = useState<string[]>([]);
  const [dbSubjects, setDbSubjects] = useState<string[]>([]);
  const [customCategory, setCustomCategory] = useState(""); // for typing a new category
  ...
  // Sync totalDuration from section durations
  useEffect(() => {
    const calculatedSum = sections.reduce((sum, sec) => sum + (sec.duration || 0), 0);
    setTotalDuration(calculatedSum || 60);
  }, [sections]);

  // Load metadata: all distinct categories + subjects in the DB
  const loadMeta = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDbSubjects(data.distinctSubjects || []);
      }
    } catch (err) {
  ...
  const handleAutoRandomize = async (sectionIdx: number) => {
    ...
    toast(
      "success",
      `Auto-assigned ${selectedIds.length} random question${selectedIds.length !== 1 ? "s" : ""} from "${sec.subject || "All subjects"}" to "${sec.name}".`,
      "Questions Randomized"
    );
  };

  // Get matching questions for modal view based on active modal filters
  ...
  const handleSaveExam = async (publishStatus: "draft" | "upcoming" | "live") => {
    ...
    if (!resolvedCategory) {
      setErrorMsg("Please select or type an exam category.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMsg(`Exam successfully created as ${publishStatus}!`);
        setTimeout(() => {
              ...
              <div
                ...
                className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-black border transition-all ${
                  step === s
                    ...
                    : step > s
                    ? "bg-blue-700 text-blue-200 border-blue-400 font-extrabold"
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-8 shadow-sm space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Set the section name, subject (for question filtering), duration and marking scheme.
                </p>
              </div>
              <button
                    ...
                    <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          disabled={idx === 0}
                          className="p-1 hover:bg-white rounded border border-slate-200 disabled:opacity-30"
                          title="Move up"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1 md:col-span-2">
                        ...
                        {dbSubjects.length > 0 ? (
                          ...
                          <input
                            ...
                            value={section.subject}
                            onChange={(e) => handleSectionFieldChange(idx, "subject", e.target.value)}
                            placeholder="e.g. General Awareness, Reasoning..."
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleSectionFieldChange(idx, "duration", Math.max(1, parseInt(e.target.value) || 0))
                          }
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleSectionFieldChange(idx, "questionCount", Math.max(1, parseInt(e.target.value) || 0))
                          }
                          className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold focus:border-[#1A56DB] outline-none"
                        />
                      </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleSectionMarkingChange(idx, "correct", parseFloat(e.target.value) || 0)
                              }
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleSectionMarkingChange(idx, "wrong", parseFloat(e.target.value) || 0)
                              }
                              className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-semibold focus:border-[#1A56DB] outline-none"
                            />
                          </div>
        ...
        {step === 3 && (
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-8 shadow-sm space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Each section shows only questions matching its subject tag. Use &quot;Pick Questions&quot; to
                manually select or &quot;Auto-Randomize&quot; to fill instantly.
              </p>
            </div>
                  ...
                  >
                    <div className="space-y-1 min-w-0">
                      <h3 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 flex-wrap">
                        {section.name}
                        {section.subject && (
                          <span className="text-[9px] bg-blue-50 text-blue-700 border border-blue-200 font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5">
                            <Tag className="h-2.5 w-2.5" /> {section.subject}
                          </span>
                        )}
                        {isTargetMet ? (
                      ...
                      <p className="text-slate-400 text-[11px]">
                        Time: {section.duration} mins | Marking: +{section.markingScheme.correct}/
                        {section.markingScheme.wrong}
                        {section.subject && (
                          <span className="ml-2 text-blue-500 font-bold">
                            · Filtered by: {section.subject}
                          </span>
                        )}
                      </p>
                    </div>
        ...
        {step === 4 && (
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-8 shadow-sm space-y-6 sm:space-y-8">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Verify the mock exam properties before publishing to the student portal.
              </p>
            </div>
            ...
            <div className="space-y-6 border border-slate-150 rounded-xl p-4 sm:p-5 bg-slate-50/20">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-b border-slate-100 pb-5">
                ...
                <div>
                  ...
                  <p className="text-xs font-black uppercase text-slate-700 mt-1">{resolvedCategory}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Total Time</p>
                  ...
                  {sections.map((sec, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 bg-white border border-slate-100 p-3 rounded-lg text-xs">
                      ...
                      <div className="text-left sm:text-right shrink-0">
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          Scheme: +{sec.markingScheme.correct}/{sec.markingScheme.wrong}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-blue-100 mt-0.5">
                  {sections[activeSectionIndex].subject
                    ? `Showing questions from subject: "${sections[activeSectionIndex].subject}". `
                    : ""}
                  Select up to {sections[activeSectionIndex].questionCount} questions. Currently selected:{" "}
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 bg-slate-50/30 min-h-[300px]">
              {loadingPool ? (
                ...
                <div className="text-center py-20 space-y-2 px-4">
                  ...
                  <p className="text-slate-400 text-[11px]">
                    {sections[activeSectionIndex]?.subject
                      ? `Upload questions with subject "${sections[activeSectionIndex].subject}" first, or change the section's subject tag.`
                      : "Upload questions to the bank first using the AI Import or Manual Entry tools."}
                  </p>
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        checked={isChecked}
                        onChange={() => {}}
                        className="mt-1 shrink-0 rounded border-slate-300 text-[#1A56DB]"
                      />
                      <div className="space-y-2 flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-1.5">
                          <span className="text-[8px] font-black uppercase bg-blue-50 text-blue-600 px-2 py-0.5 rounded border border-blue-100">
                            {q.subject}
                          </span>
